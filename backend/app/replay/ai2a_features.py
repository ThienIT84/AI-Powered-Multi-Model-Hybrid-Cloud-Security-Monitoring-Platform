...
class AI2AFlowFeatureExtractor:
    ...
    def enrich_flows(self, flows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        state = AI2AStreamingFlowFeatureExtractor(
        ...
        for batch in _same_timestamp_batches(sorted_flows):
            enriched.extend(state.enrich_batch(batch))
        return enriched

    def base_features(self, flow: dict[str, Any]) -> dict[str, Any]:
        return _base_features(flow, self.internal_networks)
...
class AI2AStreamingFlowFeatureExtractor:
    """Stateful AI2A feature extractor for live conn.log tailing."""
    ...
    def __init__(self, *, internal_cidrs: list[str] | None = None, window_seconds: float = 60.0) -> None:
        ...
        self._same_src = same_src
        self._same_src_dst = same_src_dst
        self._ssh_src_dst = ssh_src_dst
    ...
    def enrich_batch(self, batch: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for flow in batch:
            ...
            src_window = self._same_src[src_key]
            src_dst_window = self._same_src_dst[src_dst_key]
            ssh_window = self._ssh_src_dst[src_dst_key]
        ...
        for addition in additions:
            self._same_src[addition["src_key"]].append(addition)
            self._same_src_dst[addition["src_dst_key"]].append(addition)
            if addition["is_ssh"]:
                self._ssh_src_dst[addition["src_dst_key"]].append(addition)
...
def _base_features(flow: dict[str, Any], internal_networks: list[Any]) -> dict[str, Any]:
    ...
    src_role = _role(src_ip, internal_networks)
    dst_role = _role(dst_ip, internal_networks)
    ...
    direction = _direction(src_role, dst_role)
    return {
        "dst_port": dst_port,
...
def _role(value: str, internal_networks: list[Any]) -> str:
    try:
        ip = ip_address(value)
    except ValueError:
        return "external"
    return "internal" if any(ip in network for network in internal_networks) else "external"
