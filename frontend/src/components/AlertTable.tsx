...
export function AlertTable({ alerts, onSelectAlert, selectedAlertId }: AlertTableProps) {
  return (
    <div className="bg-[#0f1115] border border-white/5 rounded-xl overflow-hidden flex flex-col mb-4 shadow-2xl">
      <div className="p-3 border-b border-white/5 flex items-center justify-between bg-black/20">
        <div className="flex items-center gap-2">
           <h3 className="text-[10px] font-black text-white uppercase tracking-[0.2em]">REAL-TIME AI SECURITY EVENTS</h3>
        </div>
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-2 bg-black/40 border border-white/5 px-2 py-0.5 rounded">
                ...
                alerts.slice(0, 8).map((alert, idx) => {
                  ...
                  return (
                    <motion.tr 
                      ...
                      exit={{ opacity: 0 }}
                      className={cn(
                        "border-b border-white/5 hover:bg-white/2 transition-all group cursor-pointer relative",
                        isSelected ? "bg-red-500/8 shadow-[inset_4px_0_0_0_#ef4444]" : idx % 2 === 0 ? "bg-white/1" : ""
                      )}
                    ...
                    >
                      ...
                      <td className="px-5 py-2.5">
                        <div className="flex items-center gap-2">
                           <span className={cn("text-[10px] font-mono font-bold tracking-tight", isSelected ? "text-white" : "text-gray-300")}>{alert.sourceIp}</span>
                           <span className="text-gray-600 text-[10px]">→</span>
                           <span className="text-[10px] font-mono text-gray-400 font-bold tracking-tight">{alert.destIp}</span>
                        </div>
                      </td>
                      <td className="px-5 py-2.5">
                        <span className={cn("text-[10px] font-bold tracking-tight", isSelected ? "text-red-400" : "text-gray-200")}>{alert.attackType}</span>
                      </td>
                      <td className="px-5 py-2.5">
                         <div className="flex items-center gap-2 max-w-30">
                            <span className={cn("text-[10px] font-black font-mono w-8", isSelected ? "text-red-400" : "text-gray-100")}>{alert.confidence}</span>
                      ...
                      <td className="px-5 py-2.5">
                        <span className="text-[9px] font-bold text-gray-400 italic">AI Model (NLP-SQLi)</span>
                      </td>
                      <td className="px-5 py-2.5">
                        <span className={cn(
      ...
      <div className="p-3 border-t border-white/5 flex items-center justify-between text-[8px] font-black text-gray-500 uppercase tracking-widest">
         <span>Showing 1 to 8 of 1,247 events</span>
         <div className="flex items-center gap-1">
            <button className="px-1.5 py-1 hover:text-white transition-colors">{'<'}</button>
            <button className="w-5 h-5 flex items-center justify-center bg-blue-600 text-white rounded">1</button>
            <button className="w-5 h-5 flex items-center justify-center hover:bg-white/5 rounded transition-colors">2</button>
            <button className="w-5 h-5 flex items-center justify-center hover:bg-white/5 rounded transition-colors">3</button>
            <span>...</span>
            <button className="px-1.5 py-1 hover:text-white transition-colors">{'>'}</button>
         </div>
      </div>
