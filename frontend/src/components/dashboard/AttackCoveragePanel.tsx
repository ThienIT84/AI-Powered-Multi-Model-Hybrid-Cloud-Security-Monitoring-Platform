...
import { Shield, CheckCircle2, ShieldAlert, Sparkles, Database } from "lucide-react";
        ...
        <table className="w-full text-left border-collapse text-[8.5px]">
          <thead>
            <tr className="border-b border-border/20 text-muted-foreground uppercase text-[7.5px] font-black">
              <th className="pb-2.5">ATTACK ATTRIBUTE VECTOR</th>
              <th className="pb-2.5">SAMPLE SIZES</th>
              <th className="pb-2.5">COVERAGE FACTOR</th>
              <th className="pb-2.5">LATEST REGISTRY RECORD</th>
              <th className="pb-2.5 text-right">ONNX TRAINING MATRIX USAGE</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/10">
            {attacks.map((att) => (
              <tr key={att.type} className="hover:bg-secondary/10 transition-colors">
                <td className="py-2.5 font-bold text-foreground flex items-center gap-1.5">
                  <ShieldAlert className="w-3.5 h-3.5 text-rose-500 shrink-0" />
                  {att.type}
                </td>
                <td className="py-2.5 font-semibold text-foreground">{att.count.toLocaleString()} samples</td>
                <td className="py-2.5 font-extrabold text-cyan-400">
                  <span className="inline-block bg-cyan-950/30 px-1.5 py-0.5 border border-cyan-500/25 rounded leading-none text-[8px]">
                    {att.coverage}
                  </span>
                </td>
                <td className="py-2.5 text-muted-foreground">{att.lastCollected}</td>
                <td className="py-2.5 text-right font-bold">
                  <span className={cn(
                    "text-[7.5px] px-2 py-0.5 rounded leading-none font-black text-white shrink-0 uppercase border",
                    att.trainingUsage.includes("Pending") 
                      ? "bg-amber-950/40 border-amber-500/30 text-amber-500" 
                      : "bg-cyan-950/40 border-cyan-500/30 text-cyan-500"
                  )}>
                    {att.trainingUsage}
                  </span>
                </td>
