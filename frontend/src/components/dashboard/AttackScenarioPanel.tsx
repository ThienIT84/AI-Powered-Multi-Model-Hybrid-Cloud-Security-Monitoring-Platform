...
import { Play, Pause, Square, AlertOctagon, Terminal, HelpCircle, Flame } from "lucide-react";
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 shadow-sm select-none">
      <div className="flex items-center justify-between mb-4 border-b border-border/20 pb-2">
        ...
        <span className="text-[7.2px] bg-orange-500/10 text-orange-500 border border-orange-500/15 px-2 py-0.5 rounded uppercase font-black font-mono">
          INJECTION CAPABLE
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 font-mono">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isSelected 
                      ...
                      : "bg-[#0c0f14]/50 border-border/70 hover:border-orange-500/30 text-foreground"
                  )}
                >
                  <span className="font-extrabold truncate w-full">{sc.name}</span>
                  <div className="flex items-center justify-between text-[6.5px] mt-1 uppercase text-muted-foreground">
                    <span>{sc.type}</span>
        ...
        <div className="lg:col-span-6 bg-[#0c0f14]/80 border border-border rounded-xl p-3 flex flex-col justify-between">
          <div className="flex justify-between items-center border-b border-border/10 pb-2">
            <div>
              <span className="text-[6.5px] text-muted-foreground uppercase block font-bold leading-none">ACTIVE TEST SCENARIO</span>
            ...
            <span className={cn(
              ...
              status === "Running" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/15" :
              status === "Paused" ? "bg-amber-500/10 text-amber-500 border-amber-500/15" :
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[8px] py-3.5 leading-none">
            <div className="flex justify-between border-b border-border/10 pb-1.5">
              <span className="text-muted-foreground font-bold">START LATITUDE:</span>
              <span className="text-foreground font-semibold">10:48:02 UTC</span>
            </div>
            <div className="flex justify-between border-b border-border/10 pb-1.5">
              <span className="text-muted-foreground font-bold">END LATITUDE:</span>
              <span className="text-foreground font-semibold">11:03:02 UTC</span>
            </div>
            <div className="flex justify-between border-b border-border/10 pb-1.5">
              <span className="text-muted-foreground font-bold font-mono">ALERTS EMITTED:</span>
              <span className="text-rose-500 font-extrabold">{alertsCount} alerts</span>
            </div>
            <div className="flex justify-between border-b border-border/10 pb-1.5">
              <span className="text-muted-foreground font-bold">CAMPAIGN CORR ID:</span>
              <span className="text-cyan-400 font-extrabold">{campaignGenerated}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 border-t border-border/10 pt-2 shrink-0">
            <button
              ...
              className={cn(
                "flex-1 py-1.5 rounded-lg border text-[8px] font-bold flex items-center justify-center gap-1 transition-all cursor-pointer",
                status === "Running" ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-500" : "bg-transparent border-border hover:border-emerald-500/30 text-foreground"
              )}
            >
              <Play size={10} /> RUN
            </button>
            <button
              ...
              className={cn(
                "flex-1 py-1.5 rounded-lg border text-[8px] font-bold flex items-center justify-center gap-1 transition-all cursor-pointer",
                status === "Paused" ? "bg-amber-500/10 border-amber-500/30 text-amber-500" : "bg-transparent border-border hover:border-amber-500/30 text-foreground"
              )}
            >
              <Pause size={10} /> PAUSE
            </button>
            <button
              ...
              className={cn(
                "flex-1 py-1.5 rounded-lg border text-[8px] font-bold flex items-center justify-center gap-1 transition-all cursor-pointer",
                status === "Completed" ? "bg-rose-500/10 border-rose-500/30 text-rose-500" : "bg-transparent border-border hover:border-rose-500/30 text-foreground"
              )}
            >
