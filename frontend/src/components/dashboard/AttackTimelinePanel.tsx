...
import { Clock, ShieldAlert, Cpu, CheckCircle2, ChevronRight, RefreshCw, Layers } from "lucide-react";
          ...
          return (
            <div 
              ...
              className={cn(
                ...
                isSelected 
                  ? "border-cyan-500/35 bg-cyan-950/15 text-cyan-400" 
                  : "border-border/40 hover:bg-muted/15 hover:border-border/60 bg-background/50"
              )}
            >
              <div className="flex items-start gap-2.5 min-w-0 flex-1">
                <span className="text-[8.5px] font-bold text-muted-foreground shrink-0 mt-0.5">{ev.time}</span>
                <div className="truncate pr-1">
                  <div className="flex items-center gap-1.5 leading-none mb-1 text-[9.5px]">
                    ...
                    <span className="text-[6.5px] font-bold text-muted-foreground">({ev.ai})</span>
                  </div>
                  <p className="text-[8px] text-muted-foreground line-clamp-1 leading-snug">{ev.desc}</p>
                </div>
              </div>

              <div className="flex items-center gap-3 shrink-0 text-right">
                <div className="text-[8px] leading-tight font-medium text-muted-foreground">
                   <span>Actor: <strong className="text-foreground">{ev.actor}</strong></span>
                   <span className="block text-[6.5px] text-muted-foreground/50">Target: {ev.target}</span>
                </div>
                <span className={cn(
                  "text-[6.8px] font-black px-1.5 py-0.75 rounded border uppercase tracking-wider leading-none block text-center min-w-13.75",
                  ev.status === "Mitigated" ? "bg-emerald-500/10 border-emerald-500/25 text-emerald-500" :
                  ev.status === "Investigating" ? "bg-red-500/10 border-red-500/25 text-red-500 animate-pulse" :
                  "bg-muted border-border text-muted-foreground"
                )}>
                  {ev.status}
                </span>
                <ChevronRight size={12} className={cn("text-muted-foreground/45 transition-transform", isSelected && "transform rotate-90 text-cyan-400")} />
              </div>
            </div>
          );
        })}
