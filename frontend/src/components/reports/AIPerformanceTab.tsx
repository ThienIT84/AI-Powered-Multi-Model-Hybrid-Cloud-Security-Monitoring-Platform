    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-card border border-border p-2.5 rounded-lg shadow-2xl text-[10px] font-mono">
          ...
          <span className="text-cyan-655 dark:text-cyan-400 font-bold block">SHAP RELEVANCE: {data.score}</span>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-[9px] font-mono text-muted-foreground">
              <Activity className="w-3.5 h-3.5 text-cyan-655 dark:text-cyan-400 animate-pulse" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                <>
                  {[1, 2, 3].map((n) => (
                    <div key={n} className="bg-muted/30 p-5 rounded-xl border border-border h-85px animate-pulse flex flex-col justify-between">
                ...
                currentMetrics.map((md, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`bg-card/50 border rounded-xl p-5 hover:border-foreground/15 transition-all duration-300 flex flex-col justify-between h-86.25px relative overflow-hidden cursor-pointer ${
                        isSelected 
                    ...
                    >
                      ...
                      {isSelected && (
                        <div className="absolute top-0 right-0 p-1 bg-cyan-500/10 border-b border-l border-cyan-500/20 text-[7px] font-mono font-black text-cyan-655 dark:text-cyan-400 uppercase tracking-widest leading-none">
                          In focus
                        </div>
                      )}
                      ...
                      <div>
                        <div className="flex items-center justify-between gap-1.5 mb-2.5">
                          <span className="text-[8.5px] font-mono font-bold text-muted-foreground uppercase tracking-widest block">
                          ...
                          return (
                            <div key={key} className="space-y-1">
                              <div className="flex justify-between items-center text-[9px] font-mono font-bold uppercase tracking-wider">
                                <span className="text-muted-foreground">{key === "f1" ? "F1-Score" : key}</span>
                      ...
                      <div className="border-t border-border pt-3 flex items-center justify-between">
                        <span className="text-[8px] font-mono text-muted-foreground uppercase tracking-widest block">
        ...
        <div className="bg-card border border-border rounded-xl p-5 flex flex-col justify-between shadow-lg h-103.75px relative overflow-hidden group">
      ...
      <div className="bg-muted/40 p-4 border border-border rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-purple-500/10 text-purple-655 dark:text-purple-400 rounded-lg border border-purple-500/20">
            <Sparkles className="w-4 h-4" />
          </div>
          <div className="space-y-0.5">
            <h4 className="text-[10px] font-mono font-black text-foreground uppercase tracking-wider">
              SHAP INTERPRETABILITY HIGHLIGHT: <span className="text-cyan-655 dark:text-cyan-400 font-bold">"orig_bytes"</span>
            </h4>
            <p className="text-[9.5px] font-medium text-muted-foreground uppercase leading-relaxed max-w-2xl">
        ...
        <button className="flex items-center gap-1.5 px-3 py-1.5 bg-muted hover:bg-muted/80 text-foreground rounded border border-border font-mono text-[9px] font-bold uppercase transition duration-200 shrink-0 cursor-pointer">
