...
export function ExecutiveSummaryTab({ timeframe }: ExecutiveSummaryTabProps) {
  ...
  const renderCustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-950/95 border border-slate-800 p-3 rounded-lg shadow-2xl backdrop-blur-md text-[10px] font-mono leading-relaxed text-left">
          <div className="border-b border-slate-900 pb-1.5 mb-1.5 flex items-center justify-between gap-4">
            <span className="text-slate-400 font-bold uppercase">TIMESTAMP: {label}</span>
          ...
          <div className="space-y-1">
            {payload.map((item: any, idx: number) => (
              <div key={idx} className="flex items-center justify-between gap-4">
                ...
                <span className="text-white font-bold">{item.value} alerts</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="space-y-6">
        ...
        >
          {isEmptyStateTriggered ? (
            <>
              ...
              <span className="text-cyan-400 uppercase">SIMULATING EMPTY STATE</span>
            </>
          ) : (
            <>
              <ToggleLeft className="w-4 h-4 text-slate-500" />
              <span className="uppercase">FORCE EMPTY STATE TEST</span>
            </>
          )}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-slate-950/50 border border-slate-900 rounded-xl p-5 flex flex-col justify-between shadow-lg h-[430px] relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-20 h-[1px] bg-cyan-500/20" />
          <div className="flex justify-between items-start gap-4">
            ...
            <div className="flex items-center gap-1.5 font-mono text-[8px] text-slate-500 uppercase px-2 py-0.5 bg-slate-900 border border-slate-850 rounded">
            ...
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                  <div className="flex justify-between items-end h-[180px] px-4 gap-4">
                    {[1, 2, 3, 4, 5, 6, 7].map((n) => (
                      ...
                      >
                        <div className="w-full h-full bg-gradient-to-t from-cyan-950/20 via-transparent to-transparent" />
                      </div>
                    ))}
                  </div>
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center border border-dashed border-slate-800 rounded-lg bg-slate-900/10"
                >
                  <Cpu className="w-8 h-8 text-slate-600 animate-pulse mb-2.5" />
                  <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest block">
                    NO CORRESPONDING SEVERITY LOGS FOUND
                  </span>
                  <p className="text-[9px] text-slate-500 leading-normal max-w-xs uppercase mt-1">
        ...
        <div className="bg-slate-950/50 border border-slate-900 rounded-xl p-5 flex flex-col justify-between shadow-lg h-[430px] relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-20 h-[1px] bg-red-500/20" />
          <div className="mb-4">
            <span className="text-[9px] font-mono font-bold text-slate-400 tracking-[0.2em] uppercase block mb-1">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 scrollbar-thin scrollbar-thumb-slate-800">
            <AnimatePresence mode="wait">
              {isLoading ? (
                // Skeletons list
                <motion.div
                ...
                >
                  {[1, 2, 3, 4].map((n) => (
                    <div key={n} className="bg-slate-900/60 p-4 border border-slate-900 rounded-lg flex justify-between items-center animate-pulse">
                      ...
                      <div className="space-y-1.5 align-right">
                        ...
                        <div className="h-2.5 w-8 bg-slate-800 rounded ml-auto" />
                      </div>
                    </div>
                  ))}
                </motion.div>
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="h-full flex flex-col items-center justify-center p-6 text-center border border-dashed border-slate-800 rounded-lg"
                >
                  <Layers className="w-8 h-8 text-slate-600 animate-pulse mb-2.5" />
                  <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest block">
                    NO COMPROMISED CLOUD ASSETS
                  </span>
                  <span className="text-[8px] text-slate-500 leading-normal uppercase block mt-1">
                ...
                >
                  {activeAssetsData.map((asset, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="bg-slate-900 border border-slate-900 rounded-lg p-3 hover:border-slate-700/60 transition-all duration-300 flex items-center justify-between gap-3 group/row shadow-[inset_0_1px_1px_rgba(255,255,255,0.01)] hover:bg-slate-900/80"
                      >
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-slate-950 rounded-lg text-cyan-400 border border-slate-800 group-hover/row:border-cyan-500/30 transition-colors">
                            <Icon className="w-4 h-4" />
                          </div>
                          <div>
                            <h4 className="text-[11px] font-mono font-black text-white tracking-widest leading-normal">
                              {asset.name}
                            </h4>
                            <span className="text-[8px] font-mono font-bold text-slate-500 uppercase tracking-wider block mt-0.5">
          ...
          <div className="border-t border-slate-900 pt-4 mt-2">
            <button className="w-full flex items-center justify-center gap-2 p-2.5 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-lg text-[9px] font-mono font-bold uppercase tracking-widest border border-slate-900 hover:border-slate-705 transition duration-200 cursor-pointer">
