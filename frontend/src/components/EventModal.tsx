        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="p-6 border-b border-border bg-muted/20">
            <div className="flex items-center justify-between mb-6">
               ...
               <button onClick={onClose} className="p-2 rounded-full hover:bg-muted text-muted-foreground transition-colors transition-colors">
                 <X size={16} />
               </button>
            </div>
            ...
            <div className="space-y-3">
               <div className="flex justify-between items-center">
                 <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">SOURCE IP:</span>
                 <span className="font-mono text-red-500 font-bold">{alert.sourceIp}</span>
               </div>
               <div className="flex justify-between items-center">
                 <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">DESTINATION IP:</span>
                 <span className="font-mono text-cyan-500 font-bold">{alert.destIp}</span>
               </div>
               <div className="flex justify-between items-center">
                 <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">SERVICE:</span>
                 <span className="text-[10px] font-black text-foreground bg-muted px-2 py-0.5 rounded border border-border tracking-widest">{alert.protocol} ({alert.destPort})</span>
               </div>
            </div>
          </div>
          ...
          <div className="p-6 border-b border-border">
            ...
            <div className="bg-orange-500/5 p-4 rounded-xl border border-orange-500/20">
               ...
               <div className="text-[10px] text-muted-foreground font-medium leading-relaxed">Adversaries may attempt to exploit a software vulnerability in an Internet-facing application or system to achieve code execution or gain initial access.</div>
            </div>
          </div>
        </div>
...
function FlowStep({ label, value, status }: { label: string, value?: string, status: 'completed' | 'active' | 'confirmed' }) {
  return (
    <div className={cn(
      ...
      {status === 'completed' && <span className="text-emerald-500 bg-emerald-500/10 px-1 rounded">DECODED</span>}
